...
\maketitle
 ...
 %%% The abstract
\begin{abstract}
While the field of rigid body simulation is well researched, it is often challenging to find the right algorithm for a particular purpose. In this paper, we present a simple but functional real-time physics simulation based on rigid body dynamics with pre-defined and contact constraints, and explore its applicability in problems more often handled by mass-spring systems or position-based dynamics. On current consumer devices, real-time simulation can only provide approximate results. While these are unsuitable for most engineering purposes, there are many applications in physics assisted 3d modeling, visualizations (conventional or VR), and video games.
\end{abstract}
...
The motion of rigid bodies can be modeled using Lagrangian mechanics. This framework allows us to use both forces and constraints to control the motion of bodies.

When a high number of rigid bodies are connected using contraints, we obtain dynamic systems akin to discretized deformable bodies like ropes, hair, or cloth. While performing the simulation of such systems using rigid body dynamics is an expensive approach, it inherently provides solutions for collisions, friction, or inextendibility. In this paper, we describe a rigid body simulation framework that supports constraints necessary for approximating deformable bodies, and we evaluate the performance and stability against the classic mass-and-spring system (?) and position based dynamics (?) approaches. Real-time hair simulation is generally based on the work of Müller et al.\cite{FTL}, supporting highly dynamic simulation of a high number of inextensible hair strands, but without interaction between hair strands handled only through a density field. We compare our method of using connected rigid bodies for hair simulation against this baseline, and identify hairstyles where our approach may have benefits.
...
Adding to the diagonal of the K matrix increases its eigenvalues, and thus reduces any numerical problems.
...
\section{Solving for constrained velocities}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.8\linewidth]{sim_fabric}
