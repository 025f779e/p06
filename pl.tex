...
%\author{Paper06}
 ...
 supervisor{L\'szl\'o Sz\'ecsi\thanks{szecsi@iit.bme.hu}}
...
Figure~\ref{fig:softbody} shows a phyically animated skeletal model. The model consists of ??? rigid bodies, and the solver took ???? ms per frame.
...
Figures~\ref{fig:fabric} and~\ref{fig:fabric} shows cloth simualtion. The model consists of ??? rigid bodies, and the solution of the contraint system took ???? ms per frame. In contrast, Figure\ref{fig:pdb} shows a capture from our position-based dynamics implementation for cloth simulation, also using ??? points, but only taking ??? ms per frame. While PBD are much faster, collisions are not supported.
...
In Figure~\ref{fig:3d} three-dimensional soft-body simulation is illustrated. For ??? rigid bodies connected by ??? contraints, the solver took ???? ms per time step.
